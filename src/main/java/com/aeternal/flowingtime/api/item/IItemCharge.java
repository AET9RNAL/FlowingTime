...
public interface IItemCharge
{
    ...
    default boolean changeCharge(@Nonnull EntityPlayer player, @Nonnull ItemStack stack, @Nullable EnumHand hand) {
        ...
        if (player.isSneaking())
        {
            if (currentCharge > 0)
            {
                player.getEntityWorld().playSound(null, player.posX, player.posY, player.posZ, FLSounds.UNCHARGE, SoundCategory.PLAYERS, 1.0F, 0.5F + ((0.5F / (float)numCharges) * currentCharge));
        ...
        {
            player.getEntityWorld().playSound(null, player.posX, player.posY, player.posZ, FLSounds.CHARGE, SoundCategory.PLAYERS, 1.0F, 0.5F + ((0.5F / (float)numCharges) * currentCharge));
            stack.getTagCompound().setInteger(KEY, currentCharge + 1);
            return true;
        }
