package com.aeternal.flowingtime.datagen;

import com.aeternal.flowingtime.api.Recipes;
import com.aeternal.flowingtime.api.crafting.BaseRecipe;
import com.aeternal.flowingtime.api.crafting.BaseShapelessRecipe;
import com.aeternal.flowingtime.api.crafting.PartRecipe;
import com.aeternal.flowingtime.recipe.IInputItemStack;
import com.aeternal.flowingtime.recipe.IngredientInput;
import com.aeternal.flowingtime.api.annotation.AnnotationScanner;
...
public class RecipeProvider extends net.minecraft.data.recipes.RecipeProvider {
    ...
    protected void buildCraftingRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        for (Class<?> clazz : recipeProviders) {
            try {
                // Assuming zero-args constructor for initialization like dataset static init
                // did
        ...
        for (Map.Entry<String, Recipe<?>> entry : map.entrySet()) {
            try {
                ...
                if (recipe instanceof BaseRecipe) {
                    ...
                    ShapedRecipeBuilder shaped = ShapedRecipeBuilder.shaped(baseRecipe.getOutput().getItem(),
                            baseRecipe.getOutput().getCount());
                    ...
                    for (PartRecipe partRecipe : baseRecipe.getPartRecipe()) {
                        ...
                        if (!recipeInput.getInputs().isEmpty() && recipeInput.getInputs().get(0).hasTag()) {
                            if (recipeInput.getInputs().size() == 1) {
                                ...
                                shaped.define(character, PartialNBTIngredient
                                        .of(recipeInput.getInputs().get(0).getTag(), items.toArray(new Item[0])));
                            }
                        } else {
                    ...
                    ShapelessRecipeBuilder shaped = ShapelessRecipeBuilder.shapeless(
                            baseShapelessRecipe.getOutput().getItem(), baseShapelessRecipe.getOutput().getCount());
                    for (IInputItemStack recipeInput : baseShapelessRecipe.getRecipeInputList()) {
                        if (!recipeInput.getInputs().isEmpty() && recipeInput.getInputs().get(0).hasTag()) {
                            if (recipeInput.getInputs().size() == 1) {
                                ...
                                List<Item> items = new ArrayList<>();
                                recipeInput.getInputs().forEach(stack -> items.add(stack.getItem()));
                                shaped.requires(PartialNBTIngredient.of(recipeInput.getInputs().get(0).getTag(),
